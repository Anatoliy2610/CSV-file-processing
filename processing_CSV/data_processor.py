...
class MethodFilter(Methods):
    ...
    def get_filter_numeric_data(
        self, data_file: list, data: float, symbol: str, name_col: str
    ) -> list:
        for index, col in enumerate(data_file[0]):
            if col.strip() == name_col:
                ...
                return [
                    product
                    for product in data_file[1:]
                    if comparison[symbol](float(product[index]))
                ]
        return ["Данные отсутствуют"]

    def get_filter_data(
        self, data_file: list, symbol: str, name_col: str, data: str
    ) -> list:
        if symbol not in (">", "<", ">=", "<=", "!=", "="):
            raise ValueError("Символ (условие) в фильтрации указан неверно")
        ...
        if name_col == "brand":
            return self.get_filter_for_brand(
                data_file=data_file, brand=data, symbol=symbol
            )
        return self.get_filter_numeric_data(
            data_file=data_file, data=float(data), symbol=symbol, name_col=name_col
        )
...
class MethodAggregate(Methods):
    def get_aggreagte_data(
        self, data_file: list, agg: str, name_col: str, symbol: str
    ) -> list:
        if symbol != "=":
        ...
        for index, col in enumerate(data_file[0]):
            if col.strip() == name_col:
                if agg == "avg":
                    return [
                        round(
                            sum([float(product[index]) for product in data_file[1:]])
                            / len(data_file[1:]),
                            2,
                        )
                    ]
...
class MethodOrderBy(Methods):
    def get_order_by_data(
        self, data_file: list, order_value: str, name_col: str, symbol: str
    ):
        if symbol != "=":
